...
class MonAn:
    def __init__(self, root):
        ...
        ms_entry = Entry(lableframeLeft,width=53,textvariable=self.var_ms).grid(row=0,column=1)

        ten_lb = Label(lableframeLeft,text="Tên món",padx=2,pady=6).grid(row=1,column=0,sticky=W)
        ten_entry = Entry(lableframeLeft,width=53,textvariable=self.var_ten).grid(row=1,column=1)

        dvt_lb = Label(lableframeLeft,text="Đơn vị tính",padx=2,pady=6).grid(row=2,column=0,sticky=W)
        dvt_entry = Entry(lableframeLeft,width=53,textvariable=self.var_donvitinh).grid(row=2,column=1)

        nhom_lb = Label(lableframeLeft,text="Nhóm",padx=2,pady=6).grid(row=3,column=0,sticky=W)
        nhom_cbb = Combobox(lableframeLeft,width=50,textvariable=self.var_nhom,state="readonly")
        ...
        gia_entry = Entry(lableframeLeft,width=53,textvariable=self.var_dongia).grid(row=4,column=1)
        #button
    ...
    def them_data(self):
        conn=sqlite3.connect("Lab5.db", timeout=20)
